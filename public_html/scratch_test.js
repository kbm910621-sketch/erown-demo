$(function() {
  ...
  /* 02-D. BESPOKE OOH 3-TIER HIERARCHY & SILKY CROSSFADE */
  function updateOohVisual(img, numEng, title, guide) {
    var $photo = $('#goMainPhoto');
    if (img && $photo.attr('src') !== img) {
      $photo.addClass('fading');
      setTimeout(function() {
        $photo.attr('src', img);
        $photo.removeClass('fading');
      }, 150);
    }
  ...
  // 1. Primary Category Accordion CLICK ONLY (Hover does not change open state or image)
  $(document).on('click', '.gpi-header-btn', function(e) {
    ...
    updateOohVisual(subImg, numEng, subTitle, subGuide);
  });

  // 2. Detail Sub-Item CLICK ONLY (Hover has visual feedback only, image changes on CLICK)
  $(document).on('click', '.gds-item', function(e) {
    ...
    updateOohVisual(img, numEng, title, guide);
  });
  ...
  $(document).on('click', '.afc-btn', function() {
    ...
    if (filter === 'all') {
      ...
      // 06. PORTFOLIO LIGHTBOX MODAL TRIGGER
  var mainPortCards = [];
  var currentMainModalIdx = 0;

  function openMainModal(idx) {
    mainPortCards = $('.main-port-card:visible').toArray();
    if (mainPortCards.length === 0) mainPortCards = $('.main-port-card').toArray();
    if (idx < 0) idx = mainPortCards.length - 1;
    if (idx >= mainPortCards.length) idx = 0;
    currentMainModalIdx = idx;

    var $card = $(mainPortCards[idx]);
    var $img = $card.find('img');
    var src = $card.data('img') || $img.attr('src');
    var title = $card.data('name') || $img.attr('alt') || '프로젝트';
    var cat = $card.data('tag') || $card.find('.asps-badge, .aeg-cat-badge, .apg-tag').first().text() || '광고사례';

    $('#modalImg').attr('src', src);
    $('#modalTitle').text(title);
    $('#modalCat').text(cat);
    $('#modalCounter').text((idx + 1) + ' / ' + mainPortCards.length);

    $('#modalBackdrop').addClass('open').css('display', 'flex');
    $('body').addClass('modal-lock');
  }
  ...
  $(document).on('click', '.main-port-card', function(e) {
    ...
    mainPortCards = $('.main-port-card:visible').toArray();
    if (mainPortCards.length === 0) mainPortCards = $('.main-port-card').toArray();
    var idx = mainPortCards.indexOf(this);
    if (idx === -1) idx = 0;
    openMainModal(idx);
  });
  ...
  $(document).on('click', '#modalPrevBtn', function(e) {
    e.stopPropagation();
    openMainModal(currentMainModalIdx - 1);
  });
  ...
  $(document).on('click', '#modalClose, #modalBackdrop', function(e) {
    if (e.target === this || $(e.target).is('#modalClose')) {
      $('#modalBackdrop').removeClass('open').css('display', 'none');
      $('body').removeClass('modal-lock');
    }
  });
